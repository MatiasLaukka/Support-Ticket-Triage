...
$runtimeRoot = (
  Resolve-Path -LiteralPath $expectedRuntimeRoot -ErrorAction Stop
).ProviderPath
...
All repository, package, path, JSON, and enumeration checks finish before the
deletion loop starts. The next server start initializes
